...
SELECT 
(cv.objid +'-'+ rf.fund_objid) AS objid, (cv.controlno +'-'+ f.code) AS controlno, cv.objid AS parentid, 
...
GROUP BY nc.fund_objid, ba.objid, ba.acctid

[getIncomeLedgerItems]
SELECT 
a.fundid, a.itemacctid, SUM(a.dr) AS dr, SUM(a.cr) AS cr, 'income_ledger' AS _schemaname  
FROM   
  ( SELECT
  ...
  GROUP BY ci.item_fund_objid, ci.item_objid 
...
FROM cashreceipt_share cs 
  ...
  INNER JOIN collectionvoucher cv ON r.collectionvoucherid=cv.objid 
WHERE cv.objid = $P{collectionvoucherid} AND crv.objid IS NULL
GROUP BY ra.fund_objid, ra.objid, ia.objid 
